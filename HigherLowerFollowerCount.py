...
def higher_lower_game(play):
  print(logo)
  score = 0
  choice_a = random.choice(data)
  while play == True:
    choice_b = random.choice(data)
    while choice_b['name'] == choice_a['name']:
      choice_b = random.choice(data)
    print(f"Compare: {choice_a['name']}, a {choice_a['description']}, from {choice_a['country']}.")
    print(vs)
    print(f"Compare: {choice_b['name']}, a {choice_b['description']}, from {choice_b['country']}.")

    guess = input("Who do you think has more followers? A or B: ").lower()
    
    if guess == 'a':
      user_choice = choice_a
      non_user_choice = choice_b
    else:
      user_choice = choice_b
      non_user_choice = choice_a
    if user_choice['follower_count'] >= non_user_choice['follower_count']:
      print("Correct!")
      score += 1
  ...
  if input("Do you want to play again? Type 'y' or 'n': ").lower() == 'y':
    higher_lower_game(True)
print("Welcome to the game!!!")
higher_lower_game(play=True)
